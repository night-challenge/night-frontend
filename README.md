...
- 서버가 생성한 별자리 좌표(`points`/`connections`)를 SVG로 렌더링
- AI가 추천한 각인 이름·키워드·코멘트를 화면에 표시
- Before(원본 이동 궤적)와 After(재구성된 별자리)를 각각 시각화
...
**🎮 게임**
- 게임 소개 및 규칙 안내, 배너 슬라이드 (`GameIntro`)
- EASY/HARD 난이도 선택 및 게임 시작
- 이어하기 지원 (localStorage 기반 진행 중인 게임 감지)
- 체스판 클릭으로 기물 선택 및 합법 이동 표시
- 이동 확정 및 실시간 포인트·턴 갱신
- 승리/패배 결과 오버레이 및 게임 종료 요약

**🃏 각인**
- 게임 종료 후 별자리 생성 결과(Before/After) 미리보기
- AI 추천 이름 사용 또는 직접 이름 입력
- 별자리 다시 생성하기(재생성)
- 보유 각인 목록 조회 및 페이지네이션
- 각인 이름 재수정
- 각인 카드 획득 완료 화면

**🛍️ 제품 및 각인 신청**
- 카테고리별(가방/트래블/패션소품/라이프스타일) 제품 슬라이드 조회
- 제품 옵션(색상/용량)별 이미지·가격 전환
- 제품 상세에서 보유 각인 선택 및 페이지네이션
- 각인 색상(gold/silver/black) 선택 및 확대 미리보기
- 각인·색상 선택 완료 후 신청, 미선택 시 상황별 에러 안내

**👤 마이페이지**
- 사용자 정보 및 최근 카드 미리보기
- 저장된 카드 전체 보기 (원형 휠 UI, 드래그 인터랙션)
- 카드 상세 조회·공유·저장
- 신청한 각인 조회, 페이지네이션, 신청 취소
