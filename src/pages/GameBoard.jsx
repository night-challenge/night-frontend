...
// 서버의 status(IN_PROGRESS / WON / LOST)
// -> 화면 표시용 값으로 변환
...
// 백엔드는 '유저 이동 1회 + AI 이동 1회'를
// 한 세트로 묶어 currentTurn을 1씩 증가시킨다.
// 따라서 별도 변환 없이 그대로 표시한다.
...
function GameBoard() {
  ...
  // 현재 선택한 기물의 종류
  // 예: pawn, knight, bishop, rook, queen, king
  const [selectedPieceType, setSelectedPieceType] = useState(null)

  // 이동 가능한 칸
  const [legalMoves, setLegalMoves] = useState([])

  const [legalMovesError, setLegalMovesError] = useState(false)
  ...
  const fetchLegalMoves = useCallback(
    async (square) => {
      try {
        const res = await gameApi.legalMoves(
          gameSessionId,
          square
        )
  ...
  const handleConfirmMove = async () => {
    ...
    try {
      ...
      // ⭐ 실제로 이동한 기물 이름
      // userMove.piece / userMove.movedPiece / selectedPieceType
      // 중 서버 응답에 존재하는 값을 우선적으로 사용
      ...
      if (userMove?.pointGained > 0) {
        const label =
          getPieceLabel(userMove.captured)
  ...
  const gameResult =
    mapStatusToResult(gameState.status)
  ...
  const selectedPieceLabel =
    getPieceLabel(selectedPieceType)
    ...
    <div className="game-board-page">
      ...
      <div className="legal-moves-box">
        ...
        <p className="legal-moves-title">
          {selectedSquare
            ? `${selectedPieceLabel}${getSubjectParticle(selectedPieceLabel)} 이동 가능한 칸`
            : '이동 가능한 칸'}
        </p>
