...
const CX = -283 // 좌우 곡선 원점 x
const RADIUS = 450 // 좌우 곡선(휘어짐) 반지름 — 세로 간격엔 영향 없음, 크게 줘서 부채꼴이 확실히 보이게
const STEP = 27 // 카드 한 칸당 각도(도) — 기울기/곡선 계산용
const MAX_TILT = 45 // 기울기·좌우 곡선이 이 각도에서 더 이상 안 커짐(카드가 회전으로 서로 침범하지 않게)
const SPACING = 208 // 카드 사이 세로 간격(px, 항상 고정) — MAX_TILT(45도)에서 회전된 카드 크기(~201px)보다 살짝만 여유 있게
